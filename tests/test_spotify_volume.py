...
class TestSpotifyVolume(unittest.TestCase):
    ...
    def test_get_is_spoken_even_though_the_job_is_muted(self):
        """set_volume is mute=True because a volume change is audible on its own.
        A reported number is not — the turn must not count as a quiet success."""
        from helpers.decorators import (
            begin_tool_outcomes,
            turn_is_quiet_success,
            turn_wants_one_message,
        )

        begin_tool_outcomes()
        self.spotify.set_volume(direction="get")
        self.assertFalse(turn_is_quiet_success())
        self.assertFalse(turn_wants_one_message())

        begin_tool_outcomes()
        self.spotify.set_volume(level=40)
        self.assertTrue(turn_is_quiet_success())
        self.assertTrue(turn_wants_one_message())
