"""The settings screen writes config.yaml. That file is also hand-edited, and it
...
class TestConfigWriter(unittest.TestCase):
    ...
    def test_comments_survive_a_write(self) -> None:
        ...
        config_writer.update(self.path, {"kiosk.idle_minutes": 30})
        text = self._text()
        self.assertIn("# Wony configuration", text)
        self.assertIn("# keep this on localhost", text)
        self.assertIn("# what you call it", text)
        self.assertEqual(self._load()["kiosk"]["idle_minutes"], 30)

    def test_nested_key_keeps_its_siblings(self) -> None:
        config_writer.update(self.path, {"ai.history.max_turns": 9})
        self.assertEqual(self._load()["ai"], {"provider": None, "history": {"max_turns": 9}})

    def test_missing_key_and_block_are_created(self) -> None:
        config_writer.update(
            self.path,
            {"modules.gmail.allow_write": True, "modules.calendar.work_start_hour": 7},
        )
        data = self._load()
        self.assertIs(data["modules"]["gmail"]["allow_write"], True)
        self.assertEqual(data["modules"]["calendar"]["work_start_hour"], 7)
    ...
    def test_values_that_yaml_would_misread_are_quoted(self) -> None:
        config_writer.update(
            self.path,
            {
                ...
                "modules.home_assistant.base_url": "http://homeassistant.local:8123",
            },
        )
        data = self._load()
        ...
        self.assertEqual(
            data["modules"]["home_assistant"]["base_url"],
            "http://homeassistant.local:8123",
        )

    def test_null_round_trips(self) -> None:
        config_writer.update(self.path, {"ai.provider": None})
        self.assertIsNone(self._load()["ai"]["provider"])
...
class TestSettingsSurface(unittest.TestCase):
    ...
    def test_out_of_range_number_is_refused(self) -> None:
        with self.assertRaises(self.settings.SettingsError):
            self.settings.apply({"kiosk.idle_minutes": 9999})
    ...
    def test_every_module_in_the_picker_exists(self) -> None:
        ...
        and then silently never registers a job."""
        for key, _label, _help in self.settings.MODULES:
            with self.subTest(module=key):
                self.assertTrue(
                    os.path.exists(os.path.join(_REPO_ROOT, "modules", f"{key}.py")),
                    f"the settings screen offers '{key}', which has no modules/ file.",
                )
